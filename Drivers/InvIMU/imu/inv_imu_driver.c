                           ...
                           const inv_imu_int_state_t *it)
{
	...
	/* Zero-initialize: int1_status_en_aux1_drdy (bit 3) is not covered by
	 * inv_imu_int_state_t and would otherwise be random stack garbage. */
	memset(&intx_configx, 0, sizeof(intx_configx));
